...
if ($acao === 'listar') {
  ...
  $rows = db()->query("SELECT id, login, nome, diretoria, superintendencia, criado_em FROM usuarios ORDER BY nome")->fetchAll();
  ok(['usuarios' => $rows]);
}
...
if ($acao === 'criar') {
  ...
  db()->prepare('INSERT INTO usuarios (login, senha_hash, nome, diretoria, superintendencia) VALUES (?,?,?,?,?)')
      ->execute([$login,$hash,$nome,$dir,$sup]);
...
if ($acao === 'login') {
  ...
  ok(['usuario' => ['id'=>(int)$u['id'], 'login'=>$u['login'], 'nome'=>$u['nome'], 'diretoria'=>$u['diretoria'], 'superintendencia'=>$u['superintendencia']]]);
}
